...
namespace FSD {
    void visual(cv::Subdiv2D coneSet, SearchTree Path, 
        ...
        visualization_msgs::MarkerArray &visualTraj 
        ) {
        ...
        visualization_msgs::Marker trajectoryPoint;
        trajectoryPoint.header.frame_id = "base_link";
        trajectoryPoint.header.stamp = ros::Time();
        trajectoryPoint.ns = "red";
        trajectoryPoint.action = visualization_msgs::Marker::ADD;
        trajectoryPoint.type = visualization_msgs::Marker::LINE_STRIP;
        trajectoryPoint.scale.x = 0.1;
        trajectoryPoint.color.r = 1.0;
        trajectoryPoint.color.a = 1.0;
        geometry_msgs::Point traj_p;
        for(int i = 0; i < trajectory_.size(); i++) {
            traj_p.x = trajectory_[i].x;
            traj_p.y = trajectory_[i].y;
            trajectoryPoint.points.push_back(traj_p);
        }
        visualTraj.markers.push_back(trajectoryPoint);
