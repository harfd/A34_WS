...
void BoundaryDetectorHandle::loadParameters() {
  ...
  if (!nodeHandle_.param<std::string>("visual_traj_topic_name",
                                      visual_traj_topic_name_,
                                      "/visualization/visual_traj")) {
    ROS_WARN_STREAM("Did not load visual_traj_topic_name. Standard value is: " << visual_traj_topic_name_);
  }
  if (!nodeHandle_.param<std::string>("planning_traj_topic_name",
                                      planning_traj_topic_name_,
                                      "/planing/traj")) {
    ROS_WARN_STREAM("Did not load planning_traj_topic_name. Standard value is: " << planning_traj_topic_name_);
  }
...
void BoundaryDetectorHandle::publishToTopics() {
  ...
  visualTrajPublisher = nodeHandle_.advertise<visualization_msgs::MarkerArray>(visual_traj_topic_name_, 1);
  planningTrajPublisher = nodeHandle_.advertise<geometry_msgs::PoseArray>(planning_traj_topic_name_, 1);
}
...
void BoundaryDetectorHandle::sendMsg() {
  ...
  visualTrajPublisher.publish(boundaryDetector_.getVisualTraj());
  planningTrajPublisher.publish(boundaryDetector_.getPlanningTraj());
}
