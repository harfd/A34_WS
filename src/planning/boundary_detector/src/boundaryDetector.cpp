...
visualization_msgs::MarkerArray BoundaryDetector::getVisualTraj() { return visualTraj; }
geometry_msgs::PoseArray BoundaryDetector::getPlanningTraj() { return planningTraj;}
...
void BoundaryDetector::runAlgorithm() {
    ...
    getTraj(BestPath);
    calPlanningTraj(BestPath);

    // Visualization
    FSD::visual(coneSet, Path, boundaryDetections, BestPath,trajectory_,visualTriangles, visualTree, visualBoundary, visualPath, visualTraj);
}
...
bool BoundaryDetector::filter(fsd_common_msgs::Map &init_map) {
    ...
    if(init_map.cone_red.size() >= 1 && init_map.cone_blue.size() >= 1)
        return true;
    else 
        return false;
...
void BoundaryDetector::getTraj(std::vector<PathPoint> &BestPath) {
    fsd::Vec_f wx, wy;
    if (BestPath.size() > 0) {
        for (int i = 0; i < BestPath.size(); i++) {
            double x = BestPath[i].x;
            double y = BestPath[i].y;
            wx.push_back(x);
            wy.push_back(y);
        }
...
void BoundaryDetector::calPlanningTraj(std::vector<PathPoint> &BestPath) {
    if (BestPath.size() != 0) {
        planningTraj.poses.clear();
        for (int i = 0; i < BestPath.size(); i++) {
            geometry_msgs::Pose pose;
            pose.position.x = BestPath[i].x;
            pose.position.y = BestPath[i].y;
            planningTraj.poses.push_back(pose);
        }
    }
}
